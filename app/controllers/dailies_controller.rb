class DailiesController < ApplicationController
  ...
  def cmtda
    cmtda = Daily.find(params[:x_id]).cmtdas.create(content: params[:content])
    cmtda.save
  ...
  def cmtdes
    #특정 댓글 찾기
    cmtda = Daily.find(params[:p_id]).cmtdas.find(params[:c_id])
    cmtda.destroy
  ...
  private
    ...
    def daily_params
      params.require(:daily).permit(:category, :day, :content)
    end
end
