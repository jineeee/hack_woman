class LicensesController < ApplicationController
  ...
  def cmtli
    cmtli = License.find(params[:x_id]).cmtlis.create(content: params[:content])
    cmtli.save
  ...
  def cmtdes
    #특정 댓글 찾기
    cmtli = License.find(params[:p_id]).cmtlis.find(params[:c_id])
    cmtli.destroy
  ...
  private
    ...
    def license_params
      params.require(:license).permit(:category, :day, :content)
    end
end
