class LanguagesController < ApplicationController
  ...
  def cmtlan
    cmtlan = Language.find(params[:x_id]).cmtlans.create(content: params[:content])
    cmtlan.save
  ...
  def cmtdes
    #특정 댓글 찾기
    cmtlan = Language.find(params[:p_id]).cmtlans.find(params[:c_id])
    cmtlan.destroy
  ...
  private
    ...
    def language_params
      params.require(:language).permit(:category, :day, :content)
    end
end
