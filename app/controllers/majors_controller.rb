class MajorsController < ApplicationController
  ...
  def cmtma
    cmtma = Major.find(params[:x_id]).cmtmas.create(content: params[:content])
    cmtma.save
  ...
  def cmtdes
    #특정 댓글 찾기
    cmtma = Major.find(params[:p_id]).cmtmas.find(params[:c_id])
    cmtma.destroy
  ...
  private
    ...
    def major_params
      params.require(:major).permit(:category, :day, :content)
    end
end
